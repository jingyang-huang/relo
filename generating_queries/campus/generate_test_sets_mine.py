...
test_area = [100.0, 150.0, -50.0, 150.0] #test_area是一个范围，表示位置是否在范围内，在范围内的用于测试.可以提高模型的泛化能力
...
def construct_query_and_database_sets(base_path, folders, pointcloud_fols, filename, p):
    ...
    for folder in folders:
        ...
        df_locations = pd.read_csv(os.path.join(
            base_path,folder,gt_filename), header=0, names = ['file','northing','easting','yaw'], low_memory=False)
        
        gt_test_filename = "gt_occ_test_3m.csv"
        df_test = pd.read_csv(os.path.join(
            base_path,folder,gt_test_filename), header=0, names = ['file','northing','easting','yaw'],low_memory=False)
        ...
        database_trees.append(database_tree) #所以总共会构建3个kdtree，因为有三个occ 
    ...
    for folder in folders:
        ...
        for index,row in df_locations.iterrows():
            filename = row['file'].replace('.pcd','.npy')
    ...
    for i in range(len(database_sets)):
        ...
        for j in range(len(test_sets)):
            ...
            for key in range(len(test_sets[j].keys())):
                ...
                index = tree.query_radius(coor, r=2.0)
                
                # indices of the positive matches in database i of each query (key) in test set j
                test_sets[j][key][i] = index[0].tolist()
    
    # dump all sets to pickle files for evaluation
    output_to_file(database_sets, 'evaluation_database.pickle')
    output_to_file(test_sets, 'evaluation_query.pickle')
    output_write_to_file(database_sets, 'evaluation_database.txt')
    output_write_to_file(test_sets, 'evaluation_query.txt')
...
folders = []
velo_file = []
all_folders = sorted(os.listdir(os.path.join(BASE_DIR)))
...
construct_query_and_database_sets(BASE_DIR, folders, "/scans/",
                                  "ground_truth.csv", test_area)
