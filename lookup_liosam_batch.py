...
def QueryinTree(res_folder,frames_folder,lookupPoseFile,all_result_csv):
    ...
    for scan_id in range(len(scan_list)):  #len(scan_list)
    ...
    statistic_num = len(scan_list) #end - start +1 # len(scan_list)
    #print(query_result)
    ...
    basic_frame = pd.DataFrame({'query_result':query_result,'eval_result':eval_result,'acc_rate':acc_result}) 
