...
parser.add_argument('--batch_num_queries', type=int, default=16,
...
TRAINING_QUERIES = get_queries_dict(cfg.TRAIN_FILE) #gt_occ_0.5
TEST_QUERIES = get_queries_dict(cfg.TEST_FILE) #gt_occ_test_0.5
...
def train():
    ...
    if cfg.LOSS_FUNCTION == 'quadruplet': #是这个
    ...
    # -------------- Resume -----------------
    if FLAGS.resume:
        resume_filename = cfg.LOG_DIR + "model-resume.ckpt"
        ...
        starting_epoch = 0
        print( "\033[0;33;31m", "starting epoch is 0","\033[0m") #打印其pcd_id
    ...
    for epoch in range(starting_epoch, cfg.MAX_EPOCH):
        print(epoch)
        print()
        log_string('**** EPOCH %03d' % (epoch) + '/%3d ****' % (cfg.MAX_EPOCH))
        sys.stdout.flush()
        ...
        eval_recall = evaluate.evaluate_model(model, corr2soft) #这里老是出问题
...
def train_one_epoch(model, optimizer, corr2soft, optimizer_c2s, train_writer, loss_function, epoch):
    ...
    # Shuffle train files
    train_file_idxs = np.arange(0, len(TRAINING_QUERIES.keys())) #把训练集打乱
    np.random.shuffle(train_file_idxs)

    # iter all batch
    # BATCH_NUM_QUERIES 即每一批有多少路径 ，因为这里是1，所以相当于还是对每个数据循环一次
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES): 
        batch_keys = train_file_idxs[i *
                                     cfg.BATCH_NUM_QUERIES:(i+1)*cfg.BATCH_NUM_QUERIES] #实际上取了一个数据
        q_tuples = []
        ...
        for j in range(cfg.BATCH_NUM_QUERIES):
            ...
            if (len(TRAINING_LATENT_VECTORS) == 0):
                ...
                print("negatives length",len(negatives))
                hard_negs = get_random_hard_negatives(
                    query, negatives, num_to_take)  #hard_negs是找离negatives最近的query，所以是非常hard的negs
                print(hard_negs)
                q_tuples.append(
                ...
                query = get_feature_representation( #没问题
                    TRAINING_QUERIES[batch_keys[j]]['query'], model)
                random.shuffle(TRAINING_QUERIES[batch_keys[j]]['negatives'])
                negatives = TRAINING_QUERIES[batch_keys[j]
                                             ]['negatives'][0:sampled_neg]
                ...
                hard_negs = list(set().union(
                    HARD_NEGATIVES[batch_keys[j]], hard_negs)) #union返回并集
                print('hard', hard_negs) #negatives好像也没问题
                q_tuples.append(
                    get_query_tuple(TRAINING_QUERIES[batch_keys[j]], cfg.TRAIN_POSITIVES_PER_QUERY, cfg.TRAIN_NEGATIVES_PER_QUERY,
                                    TRAINING_QUERIES, hard_negs, other_neg=True))
                # q_tuples.append(get_rotated_tuple(TRAINING_QUERIES[batch_keys[j]],POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY, TRAINING_QUERIES, hard_negs, other_neg=True))
        ...
        for b in range(positives.shape[1]):
            for dims in range(positives.shape[2]):
                positives[0,b,dims,...] = rotation_on_SCI(positives[0,b,dims,...], (heading[0]-heading[1+b])/np.pi*180) #这个好像也不该有什么问题
