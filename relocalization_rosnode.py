...
from geometry_msgs.msg import Point, Point32,Pose2D,PoseWithCovarianceStamped
...
class Relocalizer(object):
    ...
    def getAllRange(self,lookupPoseFile):
        file = open(lookupPoseFile) 
        lines = len(file.readlines()) 
        allRange = range(lines)
        print(allRange)
        return allRange
    ...
    def __init__(self) -> None:
        self.velodyne_sub = rospy.Subscriber("/velodyne_points",
                                     PointCloud2,
                                     self.pc_callback, queue_size=1) #queue_size为1保证最新的
        self.locstatus_sub = rospy.Subscriber("/location_status",
                                     Int8,
                                     self.loc_callback, queue_size=10) #queue_size为1保证最新的
        self.repose_pub = rospy.Publisher("/initialpose", PoseWithCovarianceStamped,queue_size=1)

        self.pc = []
        self.pc_ok_flag = 0
        self.allow_pc_access= 0
        self.last_locstatus = 0
        self.scan_array = []
        self.tflistener = tf.TransformListener()
        self.last_pose = []
        
    def generate_initialpose(self,now_pose):
        pose_msg=PoseWithCovarianceStamped()
        pose_msg.header.frame_id = "map"
        pose_msg.pose.pose.position.x = now_pose[0]
        pose_msg.pose.pose.position.y = now_pose[1]
        pose_msg.pose.pose.position.z = 0.0
        quat = tf.transformations.quaternion_from_euler(0, 0, now_pose[2])
    ...
    def initial_relocalize(self):
        ...
        if( len(last_pose) != 0):
            ...
            queryRange = self.getAllRange(lookupPoseFile)
            now_pose = self.QueryinTree(res_folder,queryRange)
            print(now_pose)
            pose_msg = self.generate_initialpose(now_pose)
            
            print("publish repose")
            self.repose_pub.publish(pose_msg)
    ...
    def loc_callback(self,data):
        #print('loc data',data.data)
        try:
            now = rospy.Time.now()
            self.tflistener.waitForTransform('/map', '/velodyne', rospy.Time(0), rospy.Duration(0.1))
            (trans, rot) = self.tflistener.lookupTransform('/map', '/velodyne', rospy.Time(0))
            # print(trans[0], trans[1], trans[2], rot[0], rot[1], rot[2], rot[3])
            # print('trans',trans)
            euler = tf.transformations.euler_from_quaternion(rot)
            yaw = euler[2] / 180 *math.pi
            self.last_pose = np.array([trans[0], trans[1],yaw])
        except (tf.Exception, tf.LookupException, tf.ConnectivityException):
            print("tf echo error")
            

        if ( data.data == 1 ): #(data==1 and self.last_locstatus == 0)
            self.allow_pc_access = 1
            self.relocalize()
        elif ( data.data == 2 ): #(data==1 and self.last_locstatus == 0)
            self.allow_pc_access = 1
            self.initial_relocalize()
        self.last_locstatus = data
...
def relocalize_node():
    ...
    #     try:
    #         now = rospy.Time.now()
    #         relo.tflistener.waitForTransform('/map', '/velodyne', rospy.Time(0), rospy.Duration(0.1))
    #         (trans, rot) = relo.tflistener.lookupTransform('/map', '/velodyne', rospy.Time(0))
    #         # print(trans[0], trans[1], trans[2], rot[0], rot[1], rot[2], rot[3])
    #         print('trans',trans)
    #         relo.last_pose = np.array([trans[0], trans[1]])
    #     except (tf.Exception, tf.LookupException, tf.ConnectivityException):
    #         print("tf echo error")
    #         continue
    #     rate.sleep(0.1)
...
if __name__ == "__main__":
    ...
    all_result_csv = "./querys/result.csv"
