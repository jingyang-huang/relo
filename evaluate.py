...
def evaluate_model(model, corr2soft):
    ...
    # print("query in for --start")
    # print(DATABASE_VECTORS)
    # print(QUERY_VECTORS)
    # print(FFT_DATABASE_VECTORS)
    # print(FFT_QUERY_VECTORS)
    # print(YAW_GT_DATABASE)
    # print(YAW_GT_QUERY)
...
def get_latent_vectors(model, dict_to_process):
    ...
    if index_edge < len(dict_to_process.keys()):
        ...
        # print(outfft.shape)
        fft = fft_result.squeeze(1) #把维度1给压缩了
        #print(fft.shape)
    ...
    model.train()
    print("latent",q_output.shape,fft_output.shape,yaw_output.shape)
...
def get_recall(m, n, DATABASE_VECTORS, QUERY_VECTORS, QUERY_SETS, FFT_DATABASE_VECTORS, FFT_QUERY_VECTORS, YAW_GT_DATABASE, YAW_GT_QUERY, corr2soft):
    ...
    #print((QUERY_SETS[0][0]))

    #也是构建KDTree，然后查询
    for i in range(len(queries_output)):

        true_neighbors = QUERY_SETS[n][i]
        print('true_neighbors',true_neighbors)
        if(len(true_neighbors) == 0):
            continue
        num_evaluated += 1
        #print("to query ",np.array([queries_output[i]]))
        ...
        for j in range(len(indices[0])):
            if indices[0][j] in true_neighbors:
                ...
                recall[j] += 1
                break
        print("indices",indices[0][0:threshold])

        if len(list(set(indices[0][0:threshold]).intersection(set(true_neighbors)))) > 0: #true_neighbors在返回的indices中
            one_percent_retrieved += 1
            print("in set")
        # else:
            #print("not in set")
            #print(indices[0][0:threshold])  #list(set(indices[0][0:threshold])
...
def phase_corr_temp(a, b, device, corr2soft):
    # a: template; b: source
