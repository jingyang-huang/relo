...
def infer(input_filename):
    ...
    checkpoint = torch.load(resume_filename,map_location='cpu')
    ...
    #out_show = out.reshape(1, 32, 32) #2^10 = 1024
    ...
    # save_res = './results/{}.npy'.format(filename)
    # #print(save_res)
    # np.save(save_res, out.cpu().numpy())

    return out,fft_result

def infer_array(input_array):
    
    query = load_array_infer(input_array)
    query = np.array(query, dtype=np.float32)
    #print(query)
    model = SC.DiSCO(output_dim=cfg.FEATURE_OUTPUT_DIM)
    corr2soft = SC.Corr2Softmax(200., 0.)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_filefolder', default='./tools/submaps_pcd/',
                        help='input file folder [default: ./tools/submaps_bin/]')
    ...
    cfg.INPUT_FILEFOLDER = FLAGS.input_filefolder
    ...
    file_list = os.listdir(cfg.INPUT_FILEFOLDER)
    #file_list.sort()
    #print(file_list)
    times = 0
